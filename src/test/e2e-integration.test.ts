...
testSuite('End-to-End Integration Tests', () => {
  ...
  async function cleanupTestData() {
    try {
      ...
      if (tenantUserId || landlordUserId) {
        await supabase.from('reviews').delete().in('tenant_id', [tenantUserId, landlordUserId].filter(Boolean))
        await supabase.from('notifications').delete().in('user_id', [tenantUserId, landlordUserId].filter(Boolean))
      }
      ...
      if (tenantUserId || landlordUserId) {
        await supabase.from('user_profiles').delete().in('user_id', [tenantUserId, landlordUserId].filter(Boolean))
      }
